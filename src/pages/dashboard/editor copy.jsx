...
export function Editor() {
  ...
  const uploadFileHandle = () => {
    ...
    inputFile.addEventListener("change", async () => {
      if (inputFile.files && inputFile.files.length > 0) {
        ...
        setContentInfo({
          ...contentInfo,
          content: contentInfo.content + fileContent,
        });
      }
    });
  };
  ...
  const uploadHandler = async () => {
    ...
    let contentImage = contentImages.filter(
    ...
    if (contentImage.length > 0) {
      let contentImageUploadRes = await uploadArticleInnerImages(
        contentImage.map((item) => item.value)
      );
      contentImageUploadRes = contentImageUploadRes.data.map((item, index) => {
        contentImage[index]["newKey"] = `${
          import.meta.env.VITE_API_SERVER_URI
        }/api/file/${item}`;
        return contentImage[index];
      });
      contentImageUploadRes.forEach((element) => {
        setContentInfo({
          title: contentInfo.title,
          content: contentInfo.content.replace(element.key, element.newKey),
        });
      });
    }
      ...
      <div className="flex gap-4 items-center justify-between flex-wrap  xl:flex-col xl:items-start 4xl:flex-row">
        ...
        <div className="flex gap-2 flex-wrap flex-auto 4xl:justify-end">
          <div className="flex gap-x-4 flex-wrap">
              ...
              <Badge
                content={
                  <XMarkIcon
                    onClick={() => {
                      setImageShowUploads(
                        imageShowUploads.filter((i) => i != item)
                      );
            ...
            <Button className="whitespace-nowrap" onClick={uploadFileHandle}>
              读取文章
            </Button>
            <Button
              className="whitespace-nowrap"
      ...
      <MdEditor
        ...
        defToolbars={[
          <NormalToolbar
            ...
            trigger={<BanknotesIcon className="w-4 h-4" />}
          />,
        ]}
      />
