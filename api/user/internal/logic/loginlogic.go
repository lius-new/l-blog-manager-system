...
func (l *LoginLogic) Login(req *types.LoginRequest) (resp *types.LoginResponse, err error) {
	...
	loginResp, err := l.svcCtx.Userer.Login(
		l.ctx,
		&user.LoginUserRequest{Username: req.Username, Password: req.Password},
	)
	if err != nil {
		panic(err)
	}
	...
	_, err = l.svcCtx.Auther.SelectSecret(
		l.ctx,
		&authorization.SelectSecretRequestWithSecret{Id: loginResp.SecretId},
	)

	// 登陆成功后判断用户是否包含凭证id, 如果没有就要生成并修改用户数据, 为其添加凭证id. 凭证id可查询用户token加密的密钥
	if len(loginResp.SecretId) == 0 ||
		(err != nil && strings.Contains(err.Error(), api.ErrNotFound.Error())) {
	...
	// 生成token
	generateResp, err := l.svcCtx.Auther.GenerateToken(
		l.ctx,
		&authorization.GenerateJwtRequestWithJwt{
